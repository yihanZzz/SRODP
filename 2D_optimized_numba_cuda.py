...
Runs all combinations and generates comprehensive log
"""
...
# All the device functions remain the same as in the original code

@cuda.jit(device=True)
def get_T1_single(i, j, idx, dW1, dW2, Xbd, Vbd, uniform_St_1, uniform_Vt_1, 
                   St_1_out, St_T_out, Vt_1_out, Vt_T_out, mu, kappa, theta, sigma, r, rho, dt):
    ...
    St_T_out[0] = St_1_out[0] + (r - 0.5 * Vt_1_out[0]) * dt + math.sqrt(Vt_1_out[0]) \
        * (rho * dW1[idx,0] + math.sqrt(1 - rho**2) * dW2[idx,0]) * math.sqrt(dt)

@cuda.jit(device=True)
def compute_Xt_single(St, Vt, Xt):
...
@cuda.jit(device=True)
def compute_YtZt_single(K, St_T, Xt, betaYt, betaZ1t, betaZ2t, dW1_val, dW2_val,
                         Yt_out, Z1t_out, Z2t_out, r, dt, chi):
    ...
    Yt_out[0] = Yt_out[0] - (r * Yt_out[0] + math.sqrt(((0.1395**2)*Z1t_out[0]**2 + (0.1900**2)*Z2t_out[0]**2)*chi))*dt
    Z1t_out[0] = Yt_out[0] * dW1_val * math.sqrt(dt)
    Z2t_out[0] = Yt_out[0] * dW2_val * math.sqrt(dt)

@cuda.jit(device=True)
def searchsorted_single(Xbd, St_T, Vbd, Vt_T, noh_val):
...
@cuda.jit(device=True)
def compute_betaYt(i, j, t, K, betaY, betaZ1, betaZ2, dW1, dW2, Xbd, Vbd, 
                   uniform_St_1, uniform_Vt_1, noi_val, noh, Nt, mu, kappa, theta, 
                   sigma, r, rho, chi, dt):
    """Compute regression coefficients using streaming approach"""
    NX = 3
    XtX = cuda.local.array((3, 3), float64)
    XtY = cuda.local.array((3, 1), float64)
    XtZ1 = cuda.local.array((3, 1), float64)
    XtZ2 = cuda.local.array((3, 1), float64)
    ...
    for idx in range(noi_val):
        ...
        get_T1_single(i, j, idx, dW1, dW2, Xbd, Vbd, uniform_St_1, 
                     uniform_Vt_1, St_1, St_T, Vt_1, Vt_T, mu, kappa, theta, sigma, r, rho, dt)
        
        Xt_1 = cuda.local.array(3, float64)
        compute_Xt_single(St_1[0], Vt_1[0], Xt_1)
        ...
        if t == Nt - 2:
            ...
            indbdX, indbdV = searchsorted_single(Xbd, St_T[0], Vbd, Vt_T[0], noh)
            
            betaYt = cuda.local.array(3, float64)
            betaZ1t = cuda.local.array(3, float64)
            betaZ2t = cuda.local.array(3, float64)
            
            for jjj in range(NX):
                betaYt[jjj] = betaY[jjj, indbdX, indbdV, Nt-t-3]
                betaZ1t[jjj] = betaZ1[jjj, indbdX, indbdV, Nt-t-3]
                betaZ2t[jjj] = betaZ2[jjj, indbdX, indbdV, Nt-t-3]
            
            Xt_T = cuda.local.array(3, float64)
            compute_Xt_single(St_T[0], Vt_T[0], Xt_T)
            
            Yt = cuda.local.array(1, float64)
            Z1t = cuda.local.array(1, float64)
            Z2t = cuda.local.array(1, float64)
            
            compute_YtZt_single(K, St_T[0], Xt_T, betaYt, betaZ1t, betaZ2t, 
                               dW1[idx,0], dW2[idx,0], Yt, Z1t, Z2t, r, dt, chi)
    ...
    L = cuda.local.array((3, 3), float64)
    CholeskyInc(XtX, L)
    
    temp_identity = identity(cuda.local.array((3, 3), float64))
    temp_matrix = cuda.local.array((3, 3), float64)
    L_1 = cuda.local.array((3, 3), float64)
    
    copy_matrix(L, temp_matrix)
    invert_matrix(temp_matrix, temp_identity)
    copy_matrix(temp_identity, L_1)
    
    L_sq = cuda.local.array((3, 3), float64)
    betaY_temp = cuda.local.array((3, 1), float64)
    betaZ1_temp = cuda.local.array((3, 1), float64)
    betaZ2_temp = cuda.local.array((3, 1), float64)
    ...
    for k in range(NX):
        ...
        betaZ1[k, i, j, Nt-t-2] = betaZ1_temp[k, 0]
        betaZ2[k, i, j, Nt-t-2] = betaZ2_temp[k, 0]

@cuda.jit
def compute_betaYt_(t, K, betaY, betaZ1, betaZ2, dW1, dW2, Xbd, Vbd, 
                    uniform_St_1, uniform_Vt_1, noi_val, noh, Nt, mu, kappa, theta, 
                    sigma, r, rho, chi, dt):
    """Kernel to compute beta coefficients"""
    i, j = cuda.grid(2)
    if i < noh and j < noh:
        compute_betaYt(i, j, t, K, betaY, betaZ1, betaZ2, dW1, dW2, Xbd, Vbd,
                      uniform_St_1, uniform_Vt_1, noi_val, noh, Nt, mu, kappa, theta, 
                      sigma, r, rho, chi, dt)

def compute_betaY(K, Xbd, Vbd, betaY, betaZ1, betaZ2, threads_per_block, blocks, noi_val, 
                  noh, Nt, mu, kappa, theta, sigma, r, rho, chi, dt):
    ...
    for t in range(Nt-2, -1, -1):
        ...
        compute_betaYt_[blocks, threads_per_block](t, K, betaY, betaZ1, betaZ2, 
                                                    dW1, dW2, Xbd, Vbd, 
                                                    uniform_St_1, uniform_Vt_1, noi_val, 
                                                    noh, Nt, mu, kappa, theta, sigma, r, rho, chi, dt)
    return betaY, betaZ1, betaZ2

def P(K, Xbd, Vbd, betaY, betaZ1, betaZ2, threads_per_block, blocks, noi_val, 
      S0, V0, r, kappa, theta, sigma, rho, chi, dt, Nt, Xmin, Xmax, Vmin, Vmax, noh, mu):
    """Compute option price"""
    NX = 3
    betaY, betaZ1, betaZ2 = compute_betaY(K, Xbd, Vbd, betaY, betaZ1, betaZ2, 
    ...
    for i in range(lenT2):
        ...
        betaZ1t[i, :] = betaZ1[:, indbdX.flatten()[i], indbdV.flatten()[i], Nt-2]
        betaZ2t[i, :] = betaZ2[:, indbdX.flatten()[i], indbdV.flatten()[i], Nt-2]
    ...
    cum_gen = - r * Yt - cp.sqrt(((0.1395**2)*Z1t**2 + (0.1900**2)*Z2t**2)*chi)
    Yt = Yt + cum_gen * dt
    RBSDEvalue = cp.maximum(cp.mean(Yt), cp.maximum(K - S0, 0))
    return RBSDEvalue

def compute_(K, Xbd, Vbd, betaY, betaZ1, betaZ2, threads_per_block, blocks, iter, noi_val,
             S0, V0, r, kappa, theta, sigma, rho, chi, dt, Nt, Xmin, Xmax, Vmin, Vmax, noh, mu):
    """Run multiple iterations"""
    Pvalue = cp.zeros((iter, 1))
    for i in range(iter):
        Pvalue[i, :] = P(K, Xbd, Vbd, betaY, betaZ1, betaZ2, threads_per_block, blocks, noi_val,
                        S0, V0, r, kappa, theta, sigma, rho, chi, dt, Nt, Xmin, Xmax, Vmin, Vmax, noh, mu)
    return Pvalue
...
def run_single_config(chi, noi, noh, T_val, Nt, K_range, iterations, log_data):
    ...
    betaZ2 = cp.zeros([NX, noh, noh, Nt-1], dtype=cp.float64)
    ...
    for K in K_range:
        ...
        Pvalue = compute_(K, Xbd, Vbd, betaY, betaZ1, betaZ2, 
                         threads_per_block, blocks, iterations, noi,
                         S0, V0, r, kappa, theta, sigma, rho, chi, dt, Nt, 
                         Xmin, Xmax, Vmin, Vmax, noh, mu)
...
def main():
    ...
    chi_list = [0.2107, 0.1026, 0.0201, 0.0]
    M_list = [2000, 5000, 10000]  # noi
    J_list = [100, 200, 400]  # noh
