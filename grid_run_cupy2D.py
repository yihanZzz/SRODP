...
def main():
    ...
    log_txt = os.path.join("logs", f"grid_run_{run_id}.log")
    log_json = os.path.join("logs", f"grid_run_{run_id}.json")

    logger = setup_logger(log_txt)

    # Grid you requested
    chi_list = [0.2107, 0.1026, 0.0201, 0.0]
    M_list = [2000, 5000, 10000]     # mapped to noi
    J_list = [100, 200, 400]         # mapped to noh
    TN_list = [(0.5, 25), (0.3, 15)]
    K_range = [90, 100, 110]
    iterations = 50
    ...
    with open(log_txt, "a", encoding="utf-8") as f:
        ...
        with contextlib.redirect_stdout(tee):
            ...
            print("=" * 80)
            print("Heston RBSDE GRID run started")
            print(f"UTC time: {datetime.now(timezone.utc).isoformat()}")
            print("=" * 80)

            # Log global run parameters once
            global_params = {
                "chi_list": chi_list,
                "M_list_noi": M_list,
                "J_list_noh": J_list,
            ...
            logger.info("Global grid parameters: %s", json.dumps(global_params))

            run_counter = 0
            total_runs = len(TN_list) * len(chi_list) * len(M_list) * len(J_list)
                ...
                for chi in chi_list:
                    for J in J_list:
                        for M in M_list:
                            ...
                            print("\n" + "#" * 80)
                            print(f"GRID RUN {run_counter}/{total_runs} | T={T} Nt={Nt} | chi={chi} | J(noh)={J} | M(noi)={M}")
                            print("#" * 80)
            
                            solver = HestonRBSDE(
                                S0=100, V0=0.04, r=0.04, kappa=1.58, theta=0.03,
                                sigma=0.2, rho=-0.26, T=T, Nt=Nt, noh=J,
                                noi=M,
                                mu=1, Xmin=0, Xmax=6, Vmin=0, Vmax=1, NX=3,
                                chi=chi
                            )
                            ...
                            for K in K_range:
                                ...
                                logger.info(
                                    "Result | T=%s | Nt=%s | chi=%s | noh=%s | noi=%s | K=%s | mean=%.6f | std=%.6f | time=%.2f",
                                    T, Nt, chi, J, M, K, mean_price, std_price, elapsed
                                )
    ...
    # Save all results ONCE (one JSON)
    summary = {
        "run_id": run_id,
