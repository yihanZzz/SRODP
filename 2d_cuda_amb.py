...
def compute_YtZt(K, St_T, Xt, betaYt, Yt, betaZ1t, Z1t, betaZ2t, Z2t, dW1, dW2): #compute least square fitted value
    for i in range(noi):
        ...
        for j in range(NX):
            X += Xt[i,j]*betaYt[i,j]
            Z1 += Xt[i,j]*betaZ1t[i,j]/dt
            Z2 += Xt[i,j]*betaZ2t[i,j]/dt
        Yt[i,0] =  X
        Z1t[i,0] =  Z1
        Z2t[i,0] =  Z2
...
@cuda.jit(device=True)
def compute_Yt_final(t, K, betaY, betaZ1, betaZ2, dW1, dW2, Xbd, Vbd, St_T, Vt_T, Xt_1, Yt, Z1t, Z2t):
    if t == Nt - 2:
        for jj in range(noi):
...
@cuda.jit(device=True)
def compute_betaYt(i, j, t, K, betaY, betaZ1, betaZ2, dW1, dW2, Xbd, Vbd, uniform_St_1, uniform_Vt_1):
    ...
    compute_Yt_final(t, K, betaY, betaZ1, betaZ2, dW1, dW2, Xbd, Vbd, St_T, Vt_T, Xt_1, Yt, Z1t, Z2t)
...
def compute_betaYt_(t, K, betaY, betaZ1, betaZ2, dW1, dW2, Xbd, Vbd, uniform_St_1, uniform_Vt_1):
    ...
    i, j = cuda.grid(2)
    if i < noh and j < noh:
        compute_betaYt(i, j, t, K, betaY, betaZ1, betaZ2, dW1_, dW2_, Xbd_, Vbd_, uniform_St_1_, uniform_Vt_1_)
    #cuda.syncthreads()

def compute_betaY(K, Xbd, Vbd, betaY, betaZ1, betaZ2, threads_per_block, blocks):
...
def compute_(K, Xbd, Vbd, betaY, betaZ1, betaZ2, threads_per_block, blocks, iter):
    Pvalue = cp.zeros((iter,1))
    for i in range(iter):
        #print('iteration =', i)
        Pvalue[i,:] = P(K, Xbd, Vbd, betaY, betaZ1, betaZ2, threads_per_block, blocks)
    return Pvalue
...
for k in range(len(K_range)):
    Pvalue = compute_(K_range[k], Xbd, Vbd, betaY, betaZ1, betaZ2, threads_per_block, blocks, iter)
